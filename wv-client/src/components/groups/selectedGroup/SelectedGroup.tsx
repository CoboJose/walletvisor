...
import { GroupTransactionDTO, GroupTransactionUsersDTO, SvgIcons } from 'types/types';
...
const SelectedGroup = (): JSX.Element => {
  ...
  const getAmounts = (): number[] => {
    ...
    for (const gtDto of groupTrnDTOs) {
      const user = getGroupTransactionLoggedUser(gtDto);
      ...
      <Card className={style.card}>
        ...
        <div className={style.info}>
          <div className={style.text}>
            <span className={style.groupName} style={{ color: groupDto.group.color }}>{groupDto.group.name}</span>
          </div>
          <div className={style.text}>
