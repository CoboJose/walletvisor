...
describe('Tracker/AddTransaction', () => {
    ...
    it('should change the default category when the type is changed', () => {
        ...
        let categoryInput  = wrapper.find('select').at(1);
        ...
        categoryInput = wrapper.find('select').at(1);
    ...
    it('should display todays date at the begining', () => {
        ...
        const dateInput = wrapper.find('input').at(2);
    ...
    it('should display an error msg when the form is submitted and the data is not valid', () => {
        ...
        expect(wrapper.find('.errormsg').at(0).text()).toBe('The title must be a string')
        expect(wrapper.find('.errormsg').at(1).text()).toBe('It must be a positive number')
        expect(wrapper.find('.errormsg').at(2).text()).toBe('Must be expense or income')
        expect(wrapper.find('.errormsg').at(3).text()).toBe('Must be a valid category')
        expect(wrapper.find('.errormsg').at(4).text()).toBe('Must be a valid Date')
    })
    ...
    it('should display the correct values if a transaction is passed for update', () => {
        ...
        expect(wrapper.find('input').at(1).props().value).toBe(10)
        expect(wrapper.find('select').at(0).props().value).toBe('income')
        expect(wrapper.find('select').at(1).props().value).toBe('salary')
        expect(wrapper.find('input').at(2).props().value).toBe('2020-11-27')
    })
